...
use pumpkin::plugin::{
    player::player_teleport::PlayerTeleportEvent, Context, EventHandler, EventPriority,
};
...
static TELEPORT_COOLDOWNS: Lazy<DashMap<Uuid, Instant>> = Lazy::new(DashMap::new);
...
async fn can_teleport(player_uuid: Uuid) -> bool {
    ...
    if let Some(last_teleport) = TELEPORT_COOLDOWNS.get(&player_uuid) {
        if now.duration_since(*last_teleport) < TELEPORT_COOLDOWN_DURATION {
            return false;
        }
    }

    TELEPORT_COOLDOWNS.insert(player_uuid, now);
    true
}
...
async fn register_events(context: &Context) {
    ...
    context
        .register_event::<PlayerTeleportEvent, commands::back::BackLocationHandler>(
...
async fn on_load(&mut self, context: &Context) -> Result<(), String> {
    ...
    log::info!("Pumpkin Essentials has been loaded.");
    Ok(())
}
